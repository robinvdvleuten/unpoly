...
(function() {
  ...
  describe('up.Params', function() {
    ...
    describe('#toQuery', function() {
      ...
      it('returns the query section for the given object', function() {
        ...
        return expect(string).toEqual(`foo-key=foo${encodedSpace}value&bar-key=bar${encodedSpace}value`);
      });
      ...
      it('returns a given query string', function() {
        ...
        return expect(string).toEqual('foo=bar');
      });
      ...
      it('omits non-primitive values (like Files) from the given params', function() {
        ...
        return expect(string).toEqual('string=foo');
      });
      ...
      return it('URL-encodes plus characters', function() {
        const string = toQuery({ 'my+key': 'my+value' });
        return expect(string).toEqual('my%2Bkey=my%2Bvalue');
      });
    });
  ...
  //        expect(string).toEqual('key1=value1&key2=value2')
    ...
    describe('#toArray', function() {
      ...
      return it('does not URL-encode equal signs in keys or values', function() {
        const array = toArray({
          'my=key': 'my=value'
        });
        return expect(array).toEqual([
  ...
  //        ])
    ...
    describe('#toFormData', () => describeCapability('canInspectFormData', function() {

      it('converts undefined to an empty FormData object', function() {
        const params = undefined;
        const formData = toFormData(params);
        return expect(toArray(formData)).toEqual([]);
    });
      ...
      it('returns a FormData with the same values', function() {
        ...
        return expect(formData.get('foo')).toEqual('bar');
      });
      ...
      return it('unescapes percent-encoded characters from a query string', function() {
    ...
    describe('#toObject', function() {
      ...
      it("parses a query string of flat key/value pairs", function() {
        ...
        return expect(toObject("foo=bar&baz=")).toEqual({"foo": "bar", "baz": ""});
      });
      ...
      it('URL-decodes keys and values', function() {
        ...
        return expect(toObject("a=b&pid%3D1234=1023")).toEqual({"pid=1234": "1023", "a": "b"});
      });
        // expect(-> up.params.toObject("foo%81E=1")).toThrowError() # invalid byte sequence in UTF-8

      it('keeps the last value if the same key appears multiple time in the input', () => expect(toObject("foo=1&bar=2&foo=3")).toEqual({"foo": "3", "bar": "2"}));

      it('builds an array value if the key ends in "[]"', () => expect(toObject("foo[]=1&bar=2&foo[]=3")).toEqual({"foo[]": ["1", "3"], "bar": "2"}));

      return it('ignores keys that would overwrite an Object prototype property', function() {
        const obj = toObject("foo=bar&hasOwnProperty=baz");
        expect(obj['foo']).toEqual('bar');
        return expect(u.isFunction(obj['hasOwnProperty'])).toBe(true);
      });
    });
    ...
    describe('#add', function() {
      ...
      it('adds a single key and value', function() {
        ...
        params.add('bar', 'two');
        return expect(params.toObject()).toEqual({ foo: 'one', bar: 'two' });
    });

      return it('stores multiple entries with the same name', function() {
        const params = new up.Params({foo: 'one'});
        params.add('foo', 'two');
        return expect(params.toArray()).toEqual([
          { name: 'foo', value: 'one' },
          { name: 'foo', value: 'two' }
        ]);
    });
    ...
    describe('#get', () => describe('(with object)', function() {
      ...
      return it('returns the first matching entries if there are are multiple entries with the same name', function() {
        const params = new up.Params('foo=one&foo=two');
        const value = params.get('foo');
        return expect(value).toEqual('one');
      });
    }));
    ...
    describe('#set', function() {
      ...
      it('composes a URL from a base URL (without query section) and a query section', function() {
        ...
        return expect(params.toURL(base)).toEqual('http://foo.bar/path?key=value');
      });
      ...
      it('adds more params to a base URL that already has a query section', function() {
        ...
        return expect(params.toURL(base)).toEqual('http://foo.bar/path?key1=value1&key2=value2');
      });

      return it('does not add a question mark to the base URL if the given params are blank', function() {
        const base = 'http://foo.bar/path';
        const params = new up.Params([]);
        return expect(params.toURL(base)).toEqual('http://foo.bar/path');
      });
    });
    ...
    describe('.fromURL', function() {
      ...
      it('parses the query section from an URL, without leading question mark', function() {
        ...
        return expect(params.toQuery()).toEqual('key=value');
      });

      return it('parses no params if the URL has no query section', function() {
    ...
    return describe('.fromForm', function() {
      ...
      it('includes the focused submit button', function() {
        const $form = $fixture('form');
        const $input = $('<input type="text" name="input-key" value="input-value">').appendTo($form);
        const $submit1 = $('<button type="submit" name="submit1-key" value="submit1-value">').appendTo($form);
        const $submit2 = $('<input type="submit" name="submit2-key" value="submit2-value">').appendTo($form);
        const $submit3 = $('<input type="submit" name="submit3-key" value="submit3-value">').appendTo($form);
      ...
      it('includes a the first submit button if no button is focused', function() {
        const $form = $fixture('form');
        const $input = $('<input type="text" name="input-key" value="input-value">').appendTo($form);
        const $submit1 = $('<button type="submit" name="submit1-key" value="submit1-value">').appendTo($form);
        const $submit2 = $('<input type="submit" name="submit2-key" value="submit2-value">').appendTo($form);
      ...
      return it('excludes a submit button without a [name] attribute', function() {
        const $form = $fixture('form');
        const $input = $('<input type="text" name="input-key" value="input-value">').appendTo($form);
        const $submit = $('<button type="submit" value="submit-value">').appendTo($form);
