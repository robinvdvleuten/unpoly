...
#                    done()
        ...
        it 'closes the current modal and wait for its close animation to finish before starting the open animation of a second modal', asyncSpec (next) ->
          ...
          up.modal.config.openDuration = 15
          ...
          next =>
            # First modal is starting opening animation (will take 15 ms)
            expect(events).toEqual ['up:modal:open']
            expect($('.target')).toHaveText('response1')

          next.after 30, =>
            # First modal has completed opening animation after 15 ms
            expect(events).toEqual ['up:modal:open', 'up:modal:opened']
            expect($('.target')).toHaveText('response1')

            # We open another modal, which will cause the first modal to start closing (will take 60 ms)
          ...
          next.after 10000, =>
            console.debug('[spec] checking event list 2: %o', u.copy(events))
